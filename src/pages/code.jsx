...
const systemMessage = {
  ...
  role: "system",
  content:
    "You are a code generator named Gleam. You must answer only in markdown code snippets. Use code comments for explanations.",
};
  ...
  return (
    <form onSubmit={handleSubmit} className="poppinsMedium mb-6 w-full mt-4 ">
        ...
        <button
          ...
          className={
            sendBtn
              ...
              : "p-3 bg-[#A521FF] text-black flex my-auto text-[min(5vw,22px)] "
          }
        >
          <FaPaperPlane />
        </button>
      ...
      <div className="textGray fontSmall mt-3">
        {language ? (
          <div>Try: Simple toggle button using react hooks.</div>
        ) : (
          <div>
            Prueba: Botón de encendido y apagado simple usando react hooks.
          </div>
        )}
      </div>
    </form>
          ...
          <div className="flex flex-col">
            <div
              className={
                lengthCosmo
                  ? `hidden`
                  : `mr-4 w-[min(12vw,50px)] h-[min(12vw,50px)] rounded-full text-black flex my-auto`
              }
            >
              <BsStars className="m-auto text-[min(7vw,30px)]" />
            </div>
            <div className="px-2 pt-2 pb-4 text-black fontSmall flex">
              <ReactMarkdown
                components={{
                  pre: ({ node, ...props }) => (
                    <div className="w-full my-2 bg-black/10 p-3 rounded-lg flex text-[min(2vw,15px)]">
                      <pre {...props} />
                    </div>
                  ),
                  code: ({ node, ...props }) => (
                    <code
                      className="bg-black/10 rounded-lg p-1 fontSmall"
                      {...props}
                    />
                  ),
                }}
                className="flex flex-col leading-7 fontSmall"
              >
                {cosmoText || ""}
              </ReactMarkdown>
            </div>
...
const Code = ({ firstName, lastName, avatar, language }) => {
      ...
      {empty ? (
        <div className="lg:pl-[280px] poppinsMedium absolute fixedCenter2 z-10 flex flex-col items-center justify-center">
          <div className="space-y-6 my-auto text-[#181818]">
            ...
            <div className="fontSmall text-[#A521FF] text-center">
              {language ? (
